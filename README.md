...
graph TD
    ...
    E -- Yes --> F[タスク完了]
    E -- No --> G[プロンプトの改善]
    G --> H[プロンプトエンジニアリング技術の適用]
    H --> B
```

1. **タスクの定義:** まず、AIモデルにどのようなタスクをさせたいのかを明確に定義します。
2. **初期プロンプトの作成:** 定義したタスクに基づいて、AIモデルに対する指示を記述します。
3. **AIモデルへの入力:** 作成したプロンプトをAIモデルに入力します。
4. **出力の評価:** AIモデルが出力した結果が、期待する品質を満たしているか評価します。
5. **結果が満足いくものでない場合:** プロンプトを修正し、再度AIモデルに入力します。この改善プロセスでは、様々なプロンプトエンジニアリング技術を適用し、より良い結果を得るために繰り返し行われます。 
